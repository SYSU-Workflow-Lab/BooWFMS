...
import cn.edu.sysu.workflow.engine.core.semantics.SCXMLSemanticsImpl;
...
public class BOXMLExecutor implements BOXMLIOProcessor {
    ...
    /**
     * Add a listener to the {@link cn.edu.sysu.workflow.engine.core.model.Observable}.
     * <p/>
     *
     * @param observable The {@link cn.edu.sysu.workflow.engine.core.model.Observable} to attach the listener to.
     * @param listener   The SCXMLListener.
     */
    public void addListener(final cn.edu.sysu.workflow.engine.core.model.Observable observable, final SCXMLListener listener) {
        exctx.getNotificationRegistry().addListener(observable, listener);
    }

    /**
     * Remove this listener from the {@link cn.edu.sysu.workflow.engine.core.model.Observable}.
     *
     * @param observable The {@link cn.edu.sysu.workflow.engine.core.model.Observable}.
     * @param listener   The SCXMLListener to be removed.
     */
    public void removeListener(final cn.edu.sysu.workflow.engine.core.model.Observable observable,
    ...
    public void go() throws ModelException {
        ...
        if (StringUtils.isEmpty(this.exctx.NotifiableId)) {
        ...
        try {
            ...
            log.error("[" + this.exctx.processInstanceId + "]" + "Go BO failed, " + e);
        }
        // same as reset
        this.reset();
    }
    ...
    public void resume(String supervisorNodeId) {
        ...
        // register a new instance tree if this state-machine is the root one
        try {
            if (StringUtils.isEmpty(supervisorNodeId)) {
                RInstanceTree myTree = new RInstanceTree();
                RTreeNode nRoot = new RTreeNode(this.exctx.getStateMachine().getName(), this.NodeId, this.exctx, null);
                myTree.SetRoot(nRoot);
                InstanceManager.registerInstanceTree(this.exctx.processInstanceId, myTree);
            ...
            log.error("[" + this.exctx.processInstanceId + "]" + "Resume BO failed, " + e);
        }
    }
